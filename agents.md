...
- Stock float data is cached in `.cache/float/shares.json` and fetched from Yahoo Finance (`v7/finance/quote`). The cache refreshes stale entries (>24h) during hydration. Float is available as a field on `Tick` and `Candidate` for use in scanner filters and strategy scoring. Float data is also wired through the backtest engine (`FloatLookup` in `RunConfig`) and the optimizer (`FloatLookup` in `Params`), so all three paths (live, backtest, optimizer) have access to float values.
