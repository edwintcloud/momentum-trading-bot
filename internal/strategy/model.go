...
// CandidateDecision captures a strategy evaluation of a candidate with diagnostics.
...
// EntryPlan describes the risk parameters for a planned entry.
...
// BuildEntryPlan creates a risk plan for a candidate entry.
func BuildEntryPlan(_ config.TradingConfig, candidate domain.Candidate) (EntryPlan, bool, string) {
	if candidate.Price <= 0 || candidate.ATR <= 0 {
		return EntryPlan{}, false, "no-atr"
	}
	riskPerShare := candidate.ATR * 1.5
	if riskPerShare <= 0 {
		return EntryPlan{}, false, "zero-risk"
	}
	stopPrice := candidate.Price - riskPerShare
	if stopPrice <= 0 {
		return EntryPlan{}, false, "stop-below-zero"
	}
	return EntryPlan{
		StopPrice:    stopPrice,
		RiskPerShare: riskPerShare,
		EntryATR:     candidate.ATR,
		SetupType:    candidate.SetupType,
	}, true, ""
}

// CurrentRMultiple returns the current R-multiple for a position.
func CurrentRMultiple(position domain.Position, currentPrice float64) float64 {
	if position.RiskPerShare <= 0 {
		return 0
	}
	return (currentPrice - position.AvgPrice) / position.RiskPerShare
}

// PeakRMultiple returns the peak R-multiple for a position.
...
func featureStats(samples []TrainingSample) (map[string]float64, map[string]float64) {
	...
	for _, name := range modelFeatureOrder {
		scale := 1.0
		if len(samples) > 1 {
			scale = modelSqrt(scales[name] / float64(len(samples)-1))
		}
		if scale < 1e-6 {
			scale = 1
		}
...
func solveLinearSystem(matrix [][]float64, vector []float64) ([]float64, error) {
	...
	for pivot := 0; pivot < size; pivot++ {
		...
		for row := pivot + 1; row < size; row++ {
			if modelAbs(augmented[row][pivot]) > modelAbs(augmented[pivotRow][pivot]) {
				pivotRow = row
			}
		}
		if modelAbs(augmented[pivotRow][pivot]) < 1e-9 {
...
func modelAbs(value float64) float64 {
	if value < 0 {
		return -value
	}
	return value
...
func modelSqrt(value float64) float64 {
	if value <= 0 {
		return 0
	}
	guess := value
