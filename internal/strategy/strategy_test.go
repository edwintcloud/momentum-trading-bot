...
// Bug 2: MinShareCount floor

func TestMinShareCountFloorBumpsUp(t *testing.T) {
	cfg := config.DefaultTradingConfig()
	cfg.StartingCapital = 25000
	cfg.RiskPerTradePct = 0.005 // small risk
	cfg.MinShareCount = 100
	cfg.RegimeGatingEnabled = false
	cfg.ConfidenceSizingEnabled = false
	cfg.VolTargetSizingEnabled = true
	cfg.TargetVolPerPosition = 0.02
	cfg.MinEntryScore = 0
	cfg.MinPositionNotionalPct = 0

	runtimeState := runtime.NewState()
	pm := portfolio.NewManager(cfg)
	volEst := risk.NewVolatilityEstimator(8.0) // very high vol to force tiny qty
	s := NewStrategy(cfg, pm, runtimeState, volEst)
	...
	signal, ok := s.EvaluateCandidate(candidate)
	if !ok {
		t.Fatal("expected signal for LOWPRC candidate")
	}

	if signal.Quantity < 100 {
		t.Errorf("quantity %d should be >= MinShareCount 100", signal.Quantity)
	}
}

func TestMinShareCountDisabledNoFloor(t *testing.T) {
	cfg := config.DefaultTradingConfig()
	cfg.StartingCapital = 25000
	cfg.RiskPerTradePct = 0.005
	cfg.MinShareCount = 0 // disabled
	cfg.RegimeGatingEnabled = false
	cfg.ConfidenceSizingEnabled = false
	cfg.VolTargetSizingEnabled = true
	cfg.TargetVolPerPosition = 0.02
	...
	// With high vol and no floor, quantity should be small
	if signal.Quantity >= 100 {
		t.Errorf("with MinShareCount=0, quantity (%d) should be small due to high vol", signal.Quantity)
	}
}
